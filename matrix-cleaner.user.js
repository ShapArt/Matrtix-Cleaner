...
(function () {
    ...
    switch (op.type) {
      ...
      case CONFIG.operationTypes.ADD_SIGNER_BUNDLE:
        return [{
          ...
          status: CONFIG.status.MANUAL_REVIEW,
          reason: 'Signer bundle работает как configurable preset. Требуется проверка mapping перед apply.',
          before: {},
          after: { preset: state.signerPresetConfig },
        }];
  ...
  function reportToCsv(report) {
    const headers = ['operationType', 'itemid', 'recId', 'rowNo', 'actionType', 'status', 'reason', 'message', 'condition', 'matchedPartnerName', 'before', 'after'];
  ...
  function buildDefaultOperationFromUi(options) {
    ...
    return normalizeOperation({
      ...
      filters: {},
      payload: Object.assign({}, opts.payload || {}, { partnerName }),
      options: {
